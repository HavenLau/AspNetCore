...
export default class CircuitManager {
  public static discoverPrerenderedCircuits(document: Document): CircuitDescriptor[] {
    const commentPairs = CircuitManager.resolveCommentPairs(document);
  ...
  public static async startCircuit(connection: signalR.HubConnection): Promise<CircuitDescriptor | undefined> {
  ...
  private static resolveCommentPairs(node: Node): MarkupRegistrationTags[] {
    ...
    while (i < childrenLength) {
      ...
      const startComponent = CircuitManager.getComponentStartComment(currentChildNode);
      if (!startComponent) {
        i++;
        const childResults = CircuitManager.resolveCommentPairs(currentChildNode);
      ...
      const endComponent = CircuitManager.getComponentEndComment(startComponent, children, i + 1, childrenLength);
